...
const CalendarHeader = styled.div<{ theme: DefaultTheme; $day_count: number }>`
  position: sticky;
  z-index: 1;
  width: ${({ theme, $day_count }) =>
    String(
      $day_count *
        (theme.sizes.calendar.cellWidth + ($day_count === 1 ? 0 : 1)),
    )}px;
  top: 0;
  display: flex;
...
const CalendarBody = styled.div<{ theme: DefaultTheme; $day_count: number }>`
  width: ${({ theme, $day_count }) =>
    $day_count *
    (theme.sizes.calendar.cellWidth + ($day_count === 1 ? 0 : 1))}px;
  position: relative;
`;
...
const DivideLines = styled.div<{ theme: DefaultTheme; $day_count: number }>`
  ...
  top: -2px;
  width: ${({ theme, $day_count }) =>
    $day_count * (theme.sizes.calendar.cellWidth + 1)}px;
  height: calc(100% + 2px);
  display: flex;
`;
